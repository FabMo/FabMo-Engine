...
exports.CP = function (args) {
    ...
    var xOffset = 0;
    if (Bang !== 0 && Bang !== 180) {
        xOffset = centerX - CPstartX;
    }
    var yOffset = 0;
    if (Eang !== 90 && Eang != 270) {
        yOffset = centerY - CPstartY;
    }
...
exports.CG = function (args) {
    // note extensive parameter list; see OpenSBP Command Ref
    ...
    } else if (this.transforms != null) {
        // OR, doe we need interpolation because of general TRANSFORM active?
        if (
            this.transforms.scale.apply != false ||
            this.transforms.shearx.apply != false ||
            this.transforms.sheary.apply != false ||
            this.transforms.level.apply != false
        ) {
    ...
    for (var i = 0; i < reps; i++) {
        ...
        if (optCG === 2) {
            ...
            for (
                ...
                if (forceInterpolation) {
                    ...
                    this.interpolate_circle(
                        ...
                        endY,
                        Plg,
                        centerX - j * Pocket_StepX,
                        centerY - j * Pocket_StepY,
                        propX,
                        propY,
                        Dir,
                        optCG
            ...
            if (forceInterpolation) {
                ...
                //  -- Just a NORMAL UNIFORM MOVE USING ARCS
                var emitObj = {};
                if (Dir === 1) {
                    outStr = "G2";
                } // Clockwise circle/arc
                ...
                if (i + 1 < reps && (endX != CGstartX || endY != CGstartY)) {
                    //If an arc, pullup and jog back to the start position
    ...
    if (optCG === 4) {
        // Add bottom circle if spiral with bottom clr is specified
        if (endX != CGstartX || endY != CGstartY) {
            //If an arc, pullup and jog back to the start position
            this.emit_move("G0", { Z: safeZCG });
            this.emit_move("G0", { X: CGstartX, Y: CGstartY });
            this.emit_move("G1", { Z: currentZ, F: feedrateZ });
    ...
    if (noPullUp === 0 && currentZ !== CGstartZ) {
...
//
exports.interpolate_circle = function (
    ICstartX,
    ICstartY,
    ICstartZ,
    endX,
    ...
    if (Dir === 1) {
        ...
        if (Bang < Eang) {
            inclAng = 6.28318530717959 - Eang + Bang;
        }
        if (Bang > Eang) {
            inclAng = Bang - Eang;
        }
...
exports.CR = function (args) {
    ...
    if (optCR > 1) {
        ...
        if (Math.abs(lenX) < Math.abs(lenY)) {
            ...
            // If a square or the X is shorter, use the X length.
