...
exports.rotate = function (PtNew, angle, RotPtX, RotPtY, PtOld) {
    if (angle !== 0) {
        ...
        if ("I" in PtNew || "J" in PtNew) {
            var Ipos =
                PtNew.I === undefined ? PtOld.xIni : PtOld.xIni + PtNew.I;
            var Jpos =
                PtNew.J === undefined ? PtOld.yIni : PtOld.yIni + PtNew.J;
            var newIpos =
                cosB * (Ipos - RotPtX) - sinB * (Jpos - RotPtY) + RotPtX;
            var newJpos =
                sinB * (Ipos - RotPtX) + cosB * (Jpos - RotPtY) + RotPtY;
...
exports.scale = function (
    ...
    if (scaleX !== 1 && "I" in PtNew) {
        ("x-center offset for arcs");
        PtNew.I = scaleX * PtNew.I + scalePtI * (1 - scaleX);
    }
    if (scaleY !== 1 && "J" in PtNew) {
        ("y-center offset for arcs");
...
exports.translate = function (PtNew, MDistX, MDistY, MDistZ) {
    if (MDistX !== 0 && "X" in PtNew) {
        PtNew.X = PtNew.X + MDistX;
    }
    if (MDistY !== 0 && "Y" in PtNew) {
        PtNew.Y = PtNew.Y + MDistY;
    }
    if (MDistZ !== 0 && "Z" in PtNew) {
        PtNew.Z = PtNew.Z + MDistZ;
    }
