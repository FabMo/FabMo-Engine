...
IdleRuntime.prototype._onG2Status = function (status) {
    ...
    switch (this.machine.status.state) {
        case "not_ready":
            ...
            log.error(
                new Error(
                    "Unexpectedly encountered the 'not ready' state in IdleRuntime"
                )
            );
            break;
        ...
        case "manual":
            ...
            if (
                (status.stat === this.driver.STAT_STOP ||
                    status.stat === this.driver.STAT_END) &&
                status.hold === 0
            ) {
...
IdleRuntime.prototype.pause = function () {
    /*this.driver.feedHold();*/
};
IdleRuntime.prototype.quit = function () {
    this.driver.quit();
};
IdleRuntime.prototype.resume = function () {
    /*this.driver.resume();*/
};
