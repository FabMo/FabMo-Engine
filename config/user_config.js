...
UserConfig.prototype.verifyAndEncryptPassword = function (password, callback) {
    ...
    var pass_shasum = crypto
        .createHash("sha256")
        .update(password)
        .digest("hex"); // save encrypted password
...
UserConfig.prototype.add = function (username, password, callback) {
    if (!/^([a-zA-Z0-9]{3,20})$/.test(username)) {
        //validate username
        callback(
            "Username not valid, it should contain between 3 and 20 characters. Special characters are not authorized.",
            null
        );
        return;
    }
    this.verifyAndEncryptPassword(
        password,
        function (err, pass_shasum) {
            if (err) {
