        ...
        try {
	        if (fileName.match(/.*\.bin/i)) {
                engine.stop('firmware',function(){
                        setTimeout(function(){
                            exec("stty -F /dev/ttyACM0 1200",function(err,result){
                                if(err){
                                    throw new Error('Error putting to sleep: ' + err);
                                } else {
                                    console.log("success sleep :" + result);
                                    setTimeout(function(){
                                        exec("bossac -w -v " + filePath,function(err,result){
                                            if(err){
                                                throw new Error('Error flashing: ' + err);
                                            } else {
                                                console.log("success flash :" + result);
                                                exec("bossac -b",function(err,result){
                                                    if(err){
                                                        throw new Error('Error setting boot flag: ' + err);
                                                    } else {
                                                        console.log("success boot :" + result);
                                                        exec("bossac -R",function(err,result){
                                                            if(err){
                                                                throw new Error('Error Reseting: ' + err);
                                                            } else {
