...
function updateUpload(key, index, file) {
    if (key in UPLOAD_INDEX) {
        ...
        if (!meta) {
            throw new Error(
                "No upload metadata for file " + index + " of " + key
            );
        }
        meta.file = file;
        if (!file) {
            throw new Error("No file supplied in request.");
        }

        upload.file_count--;
        log.info(
            "Recieved file #" + index + " (" + file.name + ") for upload " + key
        );
    ...
    switch (req.method) {
        case "GET":
            ...
            var url =
                ...
                host +
                ":" +
                (config.engine.get("server_port") + 1) +
                "/" +
                path;
