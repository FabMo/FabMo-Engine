...
## Project Overview
FabMo Engine is host software for G2 motion control platforms. It manages CNC motion systems, streams G-Code, interprets OpenSBP (ShopBot language), and hosts a web-based dashboard for controlling digital fabrication tools.

**Key Technologies**: Node.js `>=18.20.0` (see `package.json` engines), Restify (REST API), Socket.IO (WebSockets), TingoDB (database), SerialPort (G2 driver), Webpack (dashboard build)

> **Sibling repos**: This engine is one of three repos that deploy together. See [`doc/system-architecture.md`](doc/system-architecture.md) for the full picture, and the companion `CLAUDE.md` files in `/fabmo-updater` (the online updater) and `/fabmo-def` (auto-profile + recovery definitions).
...
6. **Node Version**: `package.json` requires Node `>=18.20.0`. (Older docs referenced v16.14.0 — that is stale.)
