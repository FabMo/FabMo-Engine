                    ...
                    if (networkOK) {
                        ...
                        if (this.network_health_retries == 0) {
                            ...
                            this.network_health_retries =
                                NETWORK_HEALTH_RETRIES;
                            this.joinAP();
                            setImmediate(this.runWifi.bind(this));
                        } else {
                            this.network_health_retries--;
                            setTimeout(
                                this.runWifi.bind(this),
                                NETWORK_HEALTH_RETRY_INTERVAL
                            );
                        }
                    }
                }.bind(this)
...
EdisonNetworkManager.prototype._joinWifi = function (ssid, password, callback) {
    var self = this;
    log.info(
        "Attempting to join wifi network: " +
            ssid +
            " with password: " +
            password
    );
...
//    key - The network key
EdisonNetworkManager.prototype.connectToAWifiNetwork = function (
    ssid,
    key,
    callback
) {
...
EdisonNetworkManager.prototype.startDHCPServer = function (
    interface,
    callback
) {
...
EdisonNetworkManager.prototype.setIpAddress = function (
    interface,
    ip,
    callback
) {
...
EdisonNetworkManager.prototype.setNetmask = function (
    interface,
    netmask,
    callback
) {
